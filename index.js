...
Window.prototype.create = function (url) {
    ...
    if (this.setup.url) {
        this.loadURL(this.setup.url, this.setup);
    }
...
Window.prototype.loadURL = function (url, options) {
    ...
    var compiledTemplatePath = options.compiledTemplatePath;
    console.log(compiledTemplatePath);
        ...
        FileSystem.readFile(layoutFile, 'utf-8', function (error, layoutCode) {
            ...
            FileSystem.readFile(url, 'utf-8', function (error, content) {
                ...
                FileSystem.writeFile(
                    compiledTemplatePath + generatedUrl,
                    html,
                    error => {
                        if (error) {
                            ...
                            FileSystem.exists(deleteUrl, function (exists) {
                                if (exists) {
                                    ...
                                    FileSystem.unlink(deleteUrl, function (error) {
                                        if (error) {
                                            console.log(
                                                'An error ocurred updating the file' + error.message
                                            );
                                            return;
                                        }
                                    ...
                                    console.log(
                                        "This file doesn't exist, cannot delete " + deleteUrl
                                    );
                                }
                            });
                        });
